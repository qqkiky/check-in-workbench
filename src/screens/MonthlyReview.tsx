...
// 月度复盘：按 durationSeconds 汇总当月时长，展示分类占比环形图与每日堆叠柱状图（结构同年度复盘，范围收敛到单月）
export function MonthlyReview({ records, categories, entries }: Props) {
  ...
  const {
    ...
    const mr = records.filter((r) => r.date.startsWith(prefix))
    const totalSec = mr.reduce((s, r) => s + (r.durationSeconds || 0), 0)
    ...
    // 每日各分类时长（堆叠柱状图）
    const dd: Record<string, number | string>[] = Array.from({ length: daysInMonth }, (_, i) => {
      const o: Record<string, number | string> = { day: `${i + 1}` }
      categories.forEach((c) => (o[c.name] = 0))
      return o
    })
    mr.forEach((r) => {
      const d = parseInt(r.date.slice(8, 10), 10) - 1
      const cat = categories.find((c) => c.id === r.categoryId)
      const name = cat?.name || '其他'
      dd[d][name] = ((dd[d][name] as number) || 0) + (r.durationSeconds || 0)
    })

    // 本月每日打卡次数（折线图）
    const dc: { date: string; count: number }[] = Array.from({ length: daysInMonth }, (_, i) => ({
      date: `${prefix}-${String(i + 1).padStart(2, '0')}`,
      count: 0,
    }))
    mr.forEach((r) => {
      const idx = parseInt(r.date.slice(8, 10), 10) - 1
      dc[idx].count += 1
    })
  ...
  }, [records, categories, entries, prefix, year, month])
    ...
    <div className="px-3 py-3 space-y-3">
      ...
      <div className="bg-white rounded-2xl p-4 shadow-card grid grid-cols-3 gap-2">
        <div className="text-center">
          ...
          <div className="text-[11px] text-gray-500 mt-0.5">月度总时长</div>
        </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-card">
        <h3 className="text-sm font-semibold mb-3">本月每日打卡次数</h3>
        {totalCount === 0 ? (
          <div className="py-10 text-center text-sm text-gray-400">本月还没有打卡记录</div>
        ) : (
          <StatsLineChart data={dailyCount} />
        )}
        <p className="mt-1 text-[10px] text-gray-400 text-center">折线展示每日打卡总次数走势</p>
      </div>

      {/* 分类打卡次数占比环形图 */}
      <div className="bg-white rounded-2xl p-4 shadow-card">
        <h3 className="text-sm font-semibold mb-3">分类打卡次数占比</h3>
        <CategoryDonut records={monthRecords} categories={categories} />
      </div>
      ...
      <div className="bg-white rounded-2xl p-4 shadow-card">
        ...
        {categoryData.length === 0 ? (
          <div className="py-10 text-center text-sm text-gray-400">本月还没有带时长的打卡记录</div>
        ) : (
          <div className="flex items-center gap-3">
            <div className="relative shrink-0" style={{ width: 140, height: 140 }}>
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                ...
                <span className="text-[10px] text-gray-400">月度总时长</span>
              </div>
            </div>
      ...
      {/* 每日时长分布堆叠柱状图 */}
      <div className="bg-white rounded-2xl p-4 shadow-card">
        <h3 className="text-sm font-semibold mb-3">每日时长分布</h3>
        {totalSeconds === 0 ? (
        ...
        <p className="mt-2 text-[10px] text-gray-400 text-center">柱体按分类颜色堆叠，高度代表当日总投入时长</p>
      </div>
    </div>
  )
}
